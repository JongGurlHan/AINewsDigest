...
| `SendResult` | 처리 |
|---|---|
| `Success` | `DeliveryLog(SUCCESS)` 기록 |
...
| `Failed(retryable=true)` | 지수 백오프(1s, 2s, 4s)로 최대 3회 재시도. 끝내 실패면 `DeliveryLog(FAILED)` |
...
11. UNSUBSCRIBED 구독자에게는 발송하지 않는다
12. **재개 검증** — 구독자 3명 중 1명에게 이미 SUCCESS `DeliveryLog`가 있는 상태로 `send()`를 호출하면 나머지 2명에게만 발송되고, 그 1명의 `DeliveryLog`는 늘어나지 않는다
13. **EMPTY 발송 검증** — EMPTY 다이제스트를 발송하면 `sentAt`이 채워지고 `status`는 그대로 `EMPTY`다 (SENT로 바뀌지 않는다)

전원 실패 (ADR-018):

14. **구독자 3명 전원이 실패하면 `sentAt`이 null로 남고 `status`가 PENDING 그대로다.** `sentAtRecorded == false`
15. **14번 직후 같은 날짜로 `send()`를 다시 호출하면 3명 전원에게 재시도된다** (멱등성 체크에 막히지 않는다)
16. 3명 중 1명만 성공해도 `sentAt`이 채워진다
17. **활성 구독자가 0명이면 `sentAt`이 채워진다** (`totalSubscribers == 0`)
18. 재개 실행에서 스킵 1명 + 남은 2명 전원 실패면 `sentAt`이 채워진다 (`skipped > 0`)
19. **429 `retry_after`가 3600으로 오면 대기하지 않고 FAILED로 기록한 뒤 다음 구독자로 넘어간다** (배치가 멈추지 않는다)
...
- 자동 해지 카운터·스윕이나 선제적 발송 속도 제한을 만들지 마라. 이유: MVP 검수에서 제거하기로 결정했다. 죽은 구독자는 403 즉시 해지가, 레이트리밋은 429 `retry_after` 반응 처리가 충분히 막는다. 구독자 순회는 순차 HTTP 호출이라 선제 스로틀 없이도 초당 수 건 수준이다
